#include <windows.h>
#include <stdio.h>
#include <string.h>
#include <stdint.h>
#include <intrin.h>

#pragma comment(lib, "advapi32.lib")
#pragma comment(lib, "crypt32.lib")
#pragma comment(lib, "user32.lib")

// ---------------- Anti Debug (taşınabilir, MSVC x86/x64 uyumlu) ----------------

static int check_isdebuggerpresent(void) {
    return IsDebuggerPresent() ? 1 : 0;
}

static int check_remotedebugger(void) {
    BOOL present = FALSE;
    if (CheckRemoteDebuggerPresent(GetCurrentProcess(), &present)) {
        return present ? 1 : 0;
    }
    return 0;
}

static int check_timing(void) {
    unsigned int aux;
    uint64_t start = __rdtscp(&aux);
    volatile int x = 0;
    for (int i = 0; i < 10000; ++i) x += i;
    uint64_t end = __rdtscp(&aux);
    uint64_t diff = end - start;
    // Orijinal eşikler korunuyor; CPU'ya göre ayarlanabilir
    if (diff > 500000 || diff < 1000) return 1;
    return 0;
}

static int check_seh(void) {
    __try {
        // Breakpoint exception tetikleyin; debugger varsa davranış farklı olabilir
        RaiseException(EXCEPTION_BREAKPOINT, 0, 0, NULL);
    }
    __except (EXCEPTION_EXECUTE_HANDLER) {
        // Eğer exception handler çalıştıysa normal ortam varsay
        return 0;
    }
    // Eğer exception handler çalışmadıysa şüpheli
    return 1;
}

static int run_anti_debug_checks(void) {
    if (check_isdebuggerpresent()) return 1;
    if (check_remotedebugger()) return 1;
    if (check_timing()) return 1;
    if (check_seh()) return 1;
    return 0;
}

// ---------------- System fingerprint ve password üretimi ----------------

static void get_system_fingerprint(char *output, int max_len) {
    if (!output || max_len <= 1) return;

    char comp_name[256] = {0};
    DWORD comp_len = sizeof(comp_name);
    GetComputerNameA(comp_name, &comp_len);

    char win_dir[256] = {0};
    GetWindowsDirectoryA(win_dir, sizeof(win_dir));

    DWORD serial = 0;
    GetVolumeInformationA("C:\\", NULL, 0, &serial, NULL, NULL, NULL, 0);

    int cpu_info[4] = {0};
    __cpuid(cpu_info, 1);

    const char *secret_parts[] = {
        "X9kL", "2mQp", "7vRt", "4wYs",
        "1nZb", "8cDf", "3gHj", "6lKm"
    };

    char combined[512];
    _snprintf_s(combined, sizeof(combined), _TRUNCATE,
                "%s|%s|%u|%d|%s%s",
                comp_name, win_dir, serial, cpu_info[0],
                secret_parts[3], secret_parts[7]);

    // SHA-256 via CryptoAPI
    HCRYPTPROV hProv = 0;
    HCRYPTHASH hHash = 0;
    uint8_t hash[32];
    DWORD hash_len = sizeof(hash);
    BOOL ok = FALSE;

    if (CryptAcquireContextA(&hProv, NULL, NULL, PROV_RSA_AES, CRYPT_VERIFYCONTEXT)) {
        if (CryptCreateHash(hProv, CALG_SHA_256, 0, 0, &hHash)) {
            if (CryptHashData(hHash, (BYTE*)combined, (DWORD)strlen(combined), 0)) {
                if (CryptGetHashParam(hHash, HP_HASHVAL, hash, &hash_len, 0)) {
                    ok = TRUE;
                }
            }
        }
    }

    const char charset[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*";
    int charset_len = (int)(sizeof(charset) - 1);

    if (!ok) {
        // Deterministic fallback (zayıf ama tutarlı)
        size_t clen = strlen(combined);
        for (int i = 0; i < 32 && i < max_len - 1; ++i) {
            uint8_t b = (uint8_t)combined[i % (clen ? clen : 1)];
            output[i] = charset[b % charset_len];
        }
        output[32] = '\0';
    } else {
        for (int i = 0; i < 32 && i < max_len - 1; ++i) {
            output[i] = charset[hash[i] % charset_len];
        }
        output[32] = '\0';
    }

    if (hHash) CryptDestroyHash(hHash);
    if (hProv) CryptReleaseContext(hProv, 0);

    SecureZeroMemory(combined, sizeof(combined));
    SecureZeroMemory(hash, sizeof(hash));
}

// Constant-time compare
static int constant_time_compare(const char *a, const char *b, size_t len) {
    volatile unsigned int diff = 0;
    for (size_t i = 0; i < len; ++i) {
        diff |= (unsigned char)a[i] ^ (unsigned char)b[i];
    }
    return diff == 0;
}

// ---------------- Password verification ----------------

__declspec(noinline) int verify_password(const char *user_pass) {
    if (!user_pass) return 0;

    if (run_anti_debug_checks()) return 0;

    char correct[33] = {0};
    get_system_fingerprint(correct, sizeof(correct));

    size_t correct_len = strnlen(correct, sizeof(correct));
    size_t user_len = strnlen(user_pass, 256);

    if (user_len != correct_len) {
        SecureZeroMemory(correct, sizeof(correct));
        return 0;
    }

    int ok = constant_time_compare(user_pass, correct, correct_len);
    SecureZeroMemory(correct, sizeof(correct));
    return ok;
}

// ---------------- Main (örnek) ----------------

void show_flag(void) {
    MessageBoxA(NULL, "FLAG: ZERO-KNOWLEDGE-PASSWORD-2025", "ACCESS GRANTED", MB_OK | MB_ICONINFORMATION);
}

int main(void) {
    char password[256];
    printf("Enter password: ");
    if (!fgets(password, sizeof(password), stdin)) return 0;
    password[strcspn(password, "\n")] = 0;

    if (verify_password(password)) {
        show_flag();
    } else {
        printf("Wrong password or anti-debug detected.\n");
    }

    return 0;
}
